...
CSV_PATH = ROOT / "webScrapes" / "headlines.csv"
SCRAPER  = ROOT / "widget_topic_headlines_scraper.py"
...
# rgba(0,0,0,2) allows mouse events to pass through while still being "hittable"
HIT_BG = "rgba(0, 0, 0, 2)"
...
def capitalize_headline(s: str) -> str:
    s = (s or "").strip()
    if not s:
        return s
    words = s.split()
    ...
    try:
        with path.open("r", encoding="utf-8", newline="") as f:
            ...
            for row in r:
                ...
                if not src: continue
                if src not in by:
                    by[src] = []; order.append(src)
                by[src].append({"rank": rank, "topic": topic, "url": url})
    except Exception: pass
...
def _format_item(rank: int, topic: str) -> str:
    ...
    shown = _html.escape(ellipsize(capitalize_headline(topic), ELLIPSIS_LIMIT))
...
class TopicRow(QLabel):
    def __init__(self, rank: int, topic: str, url: str, get_vbar):
        ...
        self.full_topic_text = capitalize_headline(topic)
        ...
        self.setToolTip(self.full_topic_text)
        self.setAlignment(align_flag() | Qt.AlignmentFlag.AlignTop)

    def enterEvent(self, event):
        QToolTip.showText(QCursor.pos(), self.full_topic_text, self)
...
class Column(QWidget):
    def __init__(self, title: str, draggable_header: bool, on_gap_delta=None):
        ...
        self.scroller.setMouseTracking(True)
        self.scroller.viewport().setMouseTracking(True)
        ...
        self.rows_widget.setMouseTracking(True)
...
class ColumnsPane(QWidget):
    ...
    def build(self, sources_order, data_by_source, gaps_map):
        ...
        for idx, title in enumerate(self.titles):
            ...
            col = Column(title, draggable_header=draggable, on_gap_delta=make_cb(idx))
...
class HeadlinesReader(QWidget):
    ...
    def __init__(self):
        ...
        # Global flag to allow tooltips on background windows
        self.setAttribute(Qt.WidgetAttribute.WA_AlwaysShowToolTips, True)
        ...
        self.title_label = QLabel("Headlines")
        ...
        # Install app-level event filter for right-edge resize from ANY child widget
